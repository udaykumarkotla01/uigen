...
// Animated bot icon that bounces when typing
function AnimatedBot({
  ...
  const sizeClasses = size === "sm" ? "h-4.5 w-4.5" : "h-7 w-7";
  const dotSize = size === "sm" ? "w-1.5 h-1.5" : "w-2 h-2";

  return (
    <div className={cn("relative", isAnimating && "animate-bounce")}>
      <Bot
        className={cn(
          sizeClasses,
          "transition-all duration-300",
          isAnimating ? "text-blue-600 scale-110" : "text-blue-600"
        )}
      />
      {isAnimating && (
        <>
          ...
          <span
            className={cn(
              "absolute -bottom-1 -left-1 bg-blue-500 rounded-full animate-pulse",
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center">
        <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-blue-50 mb-4 shadow-sm">
          <AnimatedBot isAnimating={isLoading} />
        </div>
